...
function ViewAllRequestedOrders(props) {
	...
	async function getOrders(page) {
		if (![ROLE.SHOPKEEPER, ROLE.WORKER].includes(auth.user.role)) return history.goBack();

		const resource = auth.user.role.toLowerCase();
		const res = await Axios.GET(`/${resource}/requestedOrders/${auth.user._id}`, { page });
