...
const onSubmit = async (values, serviceCategoryId, setError, history, image) => {
	setError("");

	values = { ...values, subCategories: JSON.stringify(values.subCategories) };
	if (image) {
	...
	const res = await Axios.PUT(`/serviceCategory/${serviceCategoryId}`, formData, headers);
...
function UpdateServiceCategory(props) {
	const { history, auth, error, location, match } = props;
	...
	async function getServiceCategory() {
		...
		const data = res.data.serviceCategory;
		setServiceCategory(data);
		initialValues.category = data.category;
		initialValues.subCategories = data.subCategories;
		setLoading(false);
	}
	...
	return (
		<div className="flex flex-col btn-main min-h80">
			<div className="flex flex-col width90 align-center">
				<div className="flex flex-col align-center width50 mb-50">
					...
					{!loading && (
						<Formik
							...
							onSubmit={(values) =>
								onSubmit(values, serviceCategoryId, setError, history, image)
							}
						>
							{(formik) => {
								return (
...
export default connect(mapStateToProps, mapDispatchToProps)(UpdateServiceCategory);
